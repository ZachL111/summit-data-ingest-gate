...
`summit-data-ingest-gate` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies ingest behavior through log and snapshot fixtures, with replay consistency checks and no credentials or hosted services.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 3.
