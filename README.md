...
summit-data-ingest-gate is a C++ project for data engineering. It focuses on this technical goal: Build a C++ toolkit that studies ingest behavior through log and snapshot fixtures, with replay consistency checks and no credentials or hosted services.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 150, risk penalty 6, latency penalty 2, and weight bonus 3. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
